...
<script>
	export default {
		...
		mounted() {
			...
			window.map = new T.Map('mapDiv', config); // 地图实例
			window.map.centerAndZoom(new T.LngLat(116.40769, 39.89945), 16);
			...
			window.map.addControl(ctrl); //添加地图选择控件
 
			window.map.setMapType(window.TMAP_HYBRID_MAP); // 设置地图位地星混合图层
